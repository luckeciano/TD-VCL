...
def main(args):
    ...
    betas = [1e-3, 1e-2, 5e-2, 1e-1, 1.0, 10.0]
    steps = [1, 2, 3, 4, 5]
    ...
    for idx, n_step in enumerate(steps):
        ...
        for beta in betas:    
            ...
            for seed in seeds:
                ...
                model = NStepKLVCL(ft_size, num_classes, n_step, args.layers, 'relu', n_heads=5)
            ...
            task_plot_dfs.append(generate_plt_dfs(results, f'\u03B2 = {beta}'))
    
        plot_values(task_plot_dfs, 5, axs[idx], idx, n_step, f'n-Step = {n_step}')

    
    plt.suptitle('Split NotMNIST - N-Step TD-VCL Ablation')
    plt.tight_layout()
    plt.savefig('./split_not_mnist_nsteptdvcl_ablation.png', bbox_inches='tight')
    plt.savefig('./split_not_mnist_nsteptdvcl_ablation.eps', bbox_inches='tight')
    plt.savefig('./split_not_mnist_nsteptdvcl_ablation.pdf', bbox_inches='tight')
...
def plot_values(dfs, num_tasks, ax, idx, n_step, title):
    ...
    df_melted.to_csv(f'split_not_mnist_nstep_ablation_final_{n_step}.csv', header=True, index=False)

    sns.pointplot(x='# tasks', y='Accuracy', data=df_melted, hue='Method', errorbar='ci', marker='o', capsize=.1, ax=ax, legend=(idx == 0))

    # Set the y-axis limits and ticks
    ax.set_ylim(0.5, 1.1)
    ax.set_yticks(np.arange(0.8, 1.05, 0.1))
