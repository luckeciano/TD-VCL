...
def main(args):
    ...
    for alpha, kl_beta, lambd_logvar in combinations:
        ...
        for seed in seeds:
            ...
            vcl_trainer = UCBTrainer(model, args, device, beta=kl_beta, alpha=alpha, no_kl=False)

            test_accuracies, test_accuracies_per_task = vcl_trainer.train_eval_loop(perm_mnist, model, args, seed, break_search=True, break_search_min=0.55)
            if test_accuracies is None:
                print("Skipping config")
                break
            seed_results.append(test_accuracies)
            seed_results_per_task.append(test_accuracies_per_task)
            if test_accuracies[-1] < 0.70:
                    print("Skipping config")
                    break
