...
def main(args):
    ...
    betas = [1e-5, 1e-4, 1e-3, 5e-3, 1e-2, 5e-2]
    steps = [1, 2, 3, 5, 8, 10]
    ...
    plt.tight_layout()
    plt.savefig('./permuted_mnist_nsteptdvcl_ablation.png', bbox_inches='tight')
    plt.savefig('./permuted_mnist_nsteptdvcl_ablation.eps', bbox_inches='tight')
    plt.savefig('./permuted_mnist_nsteptdvcl_ablation.pdf', bbox_inches='tight')
...
def plot_values(dfs, num_tasks, ax, idx, n_step, title):
    ...
    df_melted.to_csv(f'permuted_mnist_nstep_ablation_final_{n_step}.csv', header=True, index=False)
