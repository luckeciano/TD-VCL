...
class UCLBayesianAlexNet64(nn.Module):
    def __init__(self, inputsize, num_heads=1, num_classes=10, lambda_logvar=-15.0, lambda_logvar_mlp=-15.0, ratio=0.5, alpha=1.0, beta=0.03, gamma=1.0):
        super(UCLBayesianAlexNet64, self).__init__()

        ncha, size, _ = inputsize
        
        self.conv1 = UCLBayesianConv2D(ncha, 64, kernel_size=5, stride=1, padding=2, lambda_logvar=lambda_logvar, ratio=ratio, alpha=alpha, beta=beta, gamma=gamma, previous_ucl_layer=None)
        self.pool1 = nn.MaxPool2d(kernel_size=2, stride=2)
        
        self.conv2 = UCLBayesianConv2D(64, 192, kernel_size=5, stride=1, padding=2, lambda_logvar=lambda_logvar, ratio=ratio, alpha=alpha, beta=beta, gamma=gamma, previous_ucl_layer=self.conv1)
        self.pool2 = nn.MaxPool2d(kernel_size=2, stride=2)
        
        self.conv3 = UCLBayesianConv2D(192, 384, kernel_size=3, stride=1, padding=1, lambda_logvar=lambda_logvar, ratio=ratio, alpha=alpha, beta=beta, gamma=gamma, previous_ucl_layer=self.conv2)
        
        self.conv4 = UCLBayesianConv2D(384, 256, kernel_size=3, stride=1, padding=1, lambda_logvar=lambda_logvar, ratio=ratio, alpha=alpha, beta=beta, gamma=gamma, previous_ucl_layer=self.conv3)
        
        self.conv5 = UCLBayesianConv2D(256, 256, kernel_size=3, stride=1, padding=1, lambda_logvar=lambda_logvar, ratio=ratio, alpha=alpha, beta=beta, gamma=gamma, previous_ucl_layer=self.conv4)
        self.pool3 = nn.MaxPool2d(kernel_size=2, stride=2)
        
        # Calculate the output size after conv and pool layers
        s = size // 8  # Size is reduced by half at each of the three pooling layers
        
        self.fc1 = UCLBayesianLinear(256 * s * s, 1024, lambda_logvar=lambda_logvar_mlp, ratio=ratio, alpha=alpha, beta=beta, gamma=gamma, previous_ucl_layer=self.conv5)
        self.drop1 = nn.Dropout(0.5)
        self.fc2 = UCLBayesianLinear(1024, 1024, lambda_logvar=lambda_logvar_mlp, ratio=ratio, alpha=alpha, beta=beta, gamma=gamma, previous_ucl_layer=self.fc1)
        self.drop2 = nn.Dropout(0.5)
